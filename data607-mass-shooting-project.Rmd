...
Ghost Guns are becoming more common, so we decided to do a sentiment analysis on what the public think about ghost guns using articles from three(3) different media outlets. So wee web-scrapped Newspaper articles with `xml2 and rvest package` and making use of the `SelectorGadget` chromes extension. The articles are:
...
The sentiment lexicon used is the `syuzhet package` which will pull out the emotions from the selected newspaper articles. The issue of controlling the expanding ghost gun accessibility has a slightly huger negative emotion than positive. The next three emotions, fear, anger, and trust, also indicates the impact of a ghost gun as a firearm.
...
From the analysis using the dataset from GVA, we found that there has been an overall upward trend in mass shootings incidents in the United States from 2014 to present with a major increase from 2019 to 2020. Also, we found that the months of July and June have the most incidents of mass shootings. Furthermore, there are more mass shootings on Sundays and Saturdays (weekends) than other weekdays. In addition, Illinois (Chicago), California, Texas, Florida, and New York are the top states with the most incidents of mass shootings, and there is an average of 5 - 6 victims per mass shooting incident.  
